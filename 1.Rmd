...
```{r Rosenbrock_2_summary}
  ...
  Lower <- quartiles[1] - 1.5*IQR
  Upper <- quartiles[2] + 1.5*IQR 
  solutions_GA_Ackley_2_clean <- (df %>% filter(algorithm == "GA", fun == "Rosenbrock", dimensions == 2, values > Lower, values < Upper))$values
  
  mean_GA_Rosenbrock_2_clean <- mean(solutions_GA_Ackley_2_clean)
...
```
<br>
Jest to jedyna z prób w których wartość p-value nie pozwala na odrzucenie hipotezy zerowej.
Usunięcie z wykresów wartośći ostających, pozwala zobaczyć ich podobieństwo
<br>
```{r plot_Rosenbrock_2_clean,message=FALSE, warning=FALSE}
  plot_PRS_Rosenbrock_2_violin <- df %>% filter(algorithm == "PRS", fun == "Rosenbrock", dimensions == 2) %>% ggplot(aes(x=dimensions, y=values, color="blue")) +  geom_violin(color="lightblue") +  geom_boxplot(width=0.1, color="lightblue")+ theme(legend.position = "none",axis.title.x = element_blank(),axis.text.x=element_blank(),
        axis.ticks.x=element_blank())+stat_summary(fun="mean", color = "black", shape=20)
  
  plot_GA_Rosenbrock_2_violin <-  df %>% filter(algorithm == "GA", fun == "Rosenbrock", dimensions == 2, values > Lower, values < Upper) %>% ggplot(aes(x=dimensions, y=values, color="red")) +  geom_violin()+ geom_boxplot(width=0.1)+ theme(legend.position = "none",axis.title.x = element_blank(),axis.text.x=element_blank(),
        axis.ticks.x=element_blank())+stat_summary(fun="mean", color = "black", shape=20)+ scale_y_continuous(trans=scales::pseudo_log_trans(base = 10))

  grid.arrange(plot_GA_Rosenbrock_2_violin, plot_PRS_Rosenbrock_2_violin, ncol=2,top = "Rosenbrock 2 dimensions cleared" )
```
<br>
